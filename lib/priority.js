...
function verdictFor(row, allStatements, level) {
  ...
  if (!threats.length) {
    return {
      ...
      reason: live.length
        ? 'на вищих пріоритетах немає бюджетних місць'
        : 'вищі пріоритети вже відпали',
    };
  }
  ...
  if (winner) {
    return {
      ...
      reason: `за пріоритетом ${winner.priority} він ${winner.position}-й на ${winner.places.budgetMax} бюджетних місць`,
  ...
  if (known.length === threats.length) {
    ...
    return {
      outcome: 'likely-stays',
      reason: `найкраще вище — ${best.position}-й на ${best.places.budgetMax} місць, не проходить`,
