...
async function rankAtDirection(statement, fallbackYear, allow) {
  ...
  // Друга, оптимістична межа. Серед тамтешніх суперників частина теж піде
  // на свої вищі пріоритети й звільнить місця — але хто саме, ми не знаємо
  // (це вимагало б опитати вже їх). Зате в списку видно пріоритет кожного,
  // а ті, для кого той напрям перший, не підуть звідти нікуди. Місце серед
  // самих лише «перших» — найкраще, на що людина може там розраховувати.
  const loyal = R.budgetApplications(data.rows)
    .filter((r) => r.priority === 1 && !r.quota)
    .sort((a, b) => b.score - a.score);

  return {
    position: ev.budgetRace.position,
    best: R.placeIn(loyal, statement.score).position,
...
async function verdictFor(row, allStatements, level, { year = null, allow = () => false } = {}) {
  ...
  // Зараз не вміщається — але вміщався б, якби тамтешні «непершопріоритетні»
  // пішли на свої вищі пріоритети. Це реальна межа невизначеності, а не
  // здогад: без опитування вже тих людей ближче підійти неможливо.
  const maybe = graded.find((g) => g.place.best != null && g.place.best <= g.place.places);
  if (maybe) {
    return {
      outcome: 'maybe-leaves',
      // Обидва числа одразу, інакше «54-й із 23 місць, але пройде» виглядає
      // безглуздо: підстава тут саме друга цифра.
      reason: `пр. ${maybe.s.priority} — ${maybe.place.position}-й із ${maybe.place.places} місць, `
        + `а серед тамтешніх першопріоритетних ${maybe.place.best}-й`,
      where: where(maybe),
      cutoff: maybe.place.cutoff ?? null,
    };
  }
...
async function simulate(data, score, { limit = 60, onProgress = null } = {}) {
  ...
  const details = [];
  let leaving = 0, likelyLeaving = 0, maybeLeaving = 0, unknown = 0;
  ...
  for (let i = 0; i < checked.length; i++) {
    ...
    if (verdict.outcome === 'maybe-leaves') maybeLeaving++;
  ...
  return {
    ...
    // Три числа — три рівні певності: підтверджений факт, розрахунок за балом
    // і найкращий випадок, коли вище теж звільняться місця.
    positionBefore: people.length + 1,
    positionAfter: people.length - leaving + 1,
    positionLikely: people.length - leaving - likelyLeaving + 1,
    positionBest: people.length - leaving - likelyLeaving - maybeLeaving + 1,
    details,
  };
}
